...
void polyvec_pqt_ntt(polyvec *r, uint8_t t)
{
  unsigned int i;
  for(i=0;i<KYBER_K;i++)
    poly_pqt_ntt(&r->vec[i], t);
}


/*************************************************
* Name:        polyvec_p_invntt_tomont
*
...
void polyvec_p_invntt_tomont(polyvec *r)
{
  unsigned int i;
  for(i=0;i<KYBER_K;i++)
    poly_p_invntt_tomont(&r->vec[i]);
}

/*************************************************
* Name:        polyvec_pqt_invntt_tomont
*
* Description: Apply inverse NTT to all elements of a vector of polynomials
*              and multiply by Montgomery factor 2^16
*
...
void polyvec_pqt_invntt_tomont(polyvec *r, uint8_t t)
{
  unsigned int i;
  for(i=0;i<KYBER_K;i++)
    poly_pqt_invntt_tomont(&r->vec[i], t);
}

/*************************************************
* Name:        polyvec_pqt_basemul_acc_montgomery
*
...
void polyvec_pqt_basemul_acc_montgomery(poly *r, const polyvec *a, const polyvec *b, uint8_t t)
{
  ...
  poly_pqt_basemul_montgomery(r, &a->vec[0], &b->vec[0], t);
  for(i=1;i<KYBER_K;i++) {
...
void polyvec_q_reduce(polyvec *r)
{
  unsigned int i;
  for(i=0;i<KYBER_K;i++)
    poly_q_reduce(&r->vec[i]);
}

/*************************************************
* Name:        polyvec_p_reduce
*
* Description: Applies Barrett reduction to each coefficient
*              of each element of a vector of polynomials;
*              for details of the Barrett reduction see comments in reduce.c
...
void polyvec_p_reduce(polyvec *r)
{
  unsigned int i;
  for(i=0;i<KYBER_K;i++)
    poly_p_reduce(&r->vec[i]);
}

/*************************************************
* Name:        polyvec_pqt_reduce
*
* Description: Applies Barrett reduction to each coefficient
*              of each element of a vector of polynomials;
*              for details of the Barrett reduction see comments in reduce.c
